...
class MazeEnv(gym.Env):
    ...
    def step(self, action):
        prob = [0.6, 0.2, 0.2] if self.is_stochastic else [1.0, 0.0, 0.0]

        if action == 0 or action == "N":
            action = np.random.choice([0, 2, 3], p=prob)

        elif action == 1 or action == "S":
            action = np.random.choice([1, 2, 3], p=prob)

        elif action == 2 or action == "E":
            action = np.random.choice([2, 0, 1], p=prob)

        elif action == 3 or action == "W":
            action = np.random.choice([3, 0, 1], p=prob)
